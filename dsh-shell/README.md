...
三平台桌面壳（Windows / macOS / Linux），v0.10.0。与 DSH 更新完全解耦：壳只负责
...
| | 状态 |
|---|---|
| `app/`（Electron 壳） | **当前唯一入口**（v0.10.0，三平台版）：窗口 + 托盘 + 服务控制 |
...
- **Windows / Linux：壳直接 spawn `dsh web` 子进程**
  ...
  - 服务 stdout/stderr 追加写入 `userData/logs/dsh-web.log`，排障先看它
  - spawn 失败（多为 dsh 未安装）→ error.html 展示 `npm i -g @deepseek-ai/dsh` 安装引导
...
（mac 拆 arm64/x64 两个产物）并自动发布 Release。
